...
<div class="max-w-2xl">
    <!-- Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">✏️ Edit Kategori</h1>
        <p class="text-gray-600 mt-2">Ubah informasi kategori wisata</p>
    </div>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="p-8">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="5"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <!-- Info Box -->
            <div class="mb-8 p-4 bg-amber-50 border-l-4 border-amber-500 rounded-lg">
                <p class="text-sm text-amber-800"><strong>⚠️ Perhatian:</strong> Slug bersifat unik, pastikan tidak ada kategori lain dengan slug yang sama.</p>
            </div>
